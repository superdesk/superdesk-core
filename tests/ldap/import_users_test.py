...
if os.environ.get("LDAP_SERVER", ""):
    ...
    class ImportUsersTestCase(TestCase):
        ...
        async def test_import_user_using_command(self, mock_ldap_connection):
            ...
            auth_user = await get_resource_service("auth_db").authenticate({"username": "barf", "password": "dummy"})
            self.assertEqual(auth_user["username"], user["user_to_import"])

            await cmd.run(user["username"], user["password"], "BARF")
            auth_user2 = await get_resource_service("auth_db").authenticate({"username": "barf", "password": "dummy"})
