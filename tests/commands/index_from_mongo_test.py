...
class IndexFromMongoTestCase(TestCase):
    def test_index_from_mongo_with_date(self):
        ...
        IndexFromMongo().run(**kwargs, date="2025-01-02")

        resp = query_elastic()
        assert resp.count() == 0

        IndexFromMongo().run(**kwargs, date="2025-01-01T08:00")

        resp = query_elastic()
        assert resp.count() == 1

        IndexFromMongo().run(**kwargs, date="2025-01-01")

        resp = query_elastic()
        assert resp.count() == 2

        with self.assertRaises(ValueError):
            IndexFromMongo().run(**kwargs, date="2025")
