...
class WENNTestCase(IsolatedAsyncioTestCase):
    ...
    def test_firstcreated(self):
        self.assertEqual(
            self.items[0].get("firstcreated"),
            datetime.datetime(year=2015, month=1, day=30, hour=0, minute=33, second=6, tzinfo=utc),
        )

    def test_versioncreated(self):
        self.assertEqual(
            self.items[0].get("versioncreated"),
            datetime.datetime(year=2015, month=1, day=30, hour=2, minute=40, second=56, tzinfo=utc),
        )
