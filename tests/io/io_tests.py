...
class UtilsTest(unittest.TestCase):
    ...
    def test_get_xml_parser_newsmlg2(self):
        ...
        self.assertIsInstance(FeedingService().get_feed_parser({'feed_parser': 'newsml2'}, etree), NewsMLTwoFeedParser)

    def test_get_xml_parser_nitf(self):
        etree = get_etree('nitf-fishing.xml')
        self.assertIsInstance(FeedingService().get_feed_parser({'feed_parser': 'nitf'}, etree), NITFFeedParser)

    def test_get_xml_parser_newsml12(self):
        etree = get_etree('afp.xml')
        self.assertIsInstance(FeedingService().get_feed_parser({'feed_parser': 'newsml12'}, etree), NewsMLOneFeedParser)
