...
TEMPLATE_FIELDS = {'template_name', 'template_type', 'schedule', 'type', 'state',
                   'last_run', 'next_run', 'template_desks', 'template_stage',
                   config.ID_FIELD, config.LAST_UPDATED, config.DATE_CREATED,
                   config.ETAG, 'task'}
KILL_TEMPLATE_NOT_REQUIRED_FIELDS = ['schedule', 'dateline', 'template_desks', 'template_stage']
...
def render_content_template(item, template):
    ...
    if template.get('template_desks') and len(template['template_desks']) == 1:
        updates['task'] = {}
        updates['task']['desk'] = template['template_desks'][0]
...
def get_item_from_template(template):
    ...
    if len(template.get('template_desks', [])) == 1:
        item['task'] = {'desk': template['template_desks'][0], 'stage': template.get('template_stage')}
