...
class CommonClient:
    ...
    def validate_scope(self, scope):
        ...
        if not scope.issubset(allowed_scopes):
            self.parser.error(
                "invalid scopes: {invalid_scopes}\nvalid scopes are: {allowed_scopes}"
                .format(
                    invalid_scopes=', '.join(scope - allowed_scopes),
...
class RegisterClient(superdesk.Command, CommonClient):
    ...
    option_list = [
        superdesk.Option(
            '--client-id', '-i', dest='client_id',
            help="ObjectId compatible client id (keep empty to generate one)"),
        superdesk.Option(
            '--password', '-p', nargs='?', const='',
            help="client password (keep empty to generate it)"),
        superdesk.Option(
            '--scope', '-s', action='append', default=[],
...
class UpdateClient(superdesk.Command, CommonClient):
    ...
    option_list = [
        superdesk.Option(
            '--name', '-n', help="change client name"),
        # empty string is used to re-generate password
        superdesk.Option(
            '--password', '-p', nargs='?', const='',
            help="update password (keep empty to re-generate it)"),
        superdesk.Option(
            '--scope', '-s', action='append', default=[],
            help='scopes allowed (one or more of {allowed_scopes})'.format(
                allowed_scopes=', '.join(allowed_scopes))),
