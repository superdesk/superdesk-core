...
from superdesk.core import get_current_app, get_app_config, get_config
...
async def create_renditions_in_celery(
    image_id: str,
    rendition_config: dict,
    file_type: str,
    resource: str = "upload",
    delete_all_on_error: bool = False,
    insert_metadata: bool = True,
    temporary: bool = True,
    content_type: str | None = None,
) -> dict:
    task_result: AsyncTaskResult[dict] | dict = await create_renditions_in_celery_task.delay(
        image_id,
        rendition_config,
        file_type,
        resource,
    ...
    return await task_result.get_result_async() if isinstance(task_result, AsyncTaskResult) else task_result


@shared_task(store_async_result=True)
async def create_renditions_in_celery_task(
    image_id: str,
    rendition_config: dict,
    file_type: str,
    resource: str = "upload",
    ...
    inserted: list[str] = []
    try:
        return generate_renditions(
            original_content,
            image_id,
            inserted,
            file_type,
            content_type or media_file.content_type,
            rendition_config,
        ...
        if delete_all_on_error:
            for file_id in inserted:
                await app.media.delete_async(file_id)
        raise SuperdeskApiError.internalError(gettext("Generating renditions failed"), exception=error)


# Deprecated: Prefer to use `create_renditions_in_celery` instead
...
# Deprecated: Use `get_renditions_spec_async` instead
...
async def get_renditions_spec_async(without_internal_renditions=False, no_custom_crops=False) -> dict:
    """Return the list of the needed renditions.
    ...
    # renditions required by superdesk
    if not without_internal_renditions:
        rendition_spec = deepcopy(get_config(dict, "RENDITIONS", {}).get("picture", {}))

    if not no_custom_crops:
        # load custom renditions sizes
        custom_crops = await get_resource_service("vocabularies").find_one_async(req=None, _id="crop_sizes")
        if custom_crops:
            for crop in custom_crops.get("items"):
                if crop.get("is_active"):
                    rendition_spec[crop["name"]] = crop
...
async def download_file_from_feeding_service_or_directly_async(
    url: str, feeding_service: Any | None = None, **kwargs
) -> tuple[BytesIO, str, str]:
    if feeding_service is not None:
        if hasattr(feeding_service, "download_file_async"):
            return await feeding_service.download_file_async(url, **kwargs)
        elif hasattr(feeding_service, "download_file"):
            return feeding_service.download_file(url, **kwargs)
    return download_file_from_url(url, kwargs)


# Deprecated: Use `update_renditions_async` instead
...
async def update_renditions_async(
    item: dict,
    href: str,
    old_item: dict | None,
    request_kwargs: dict | None = None,
    feeding_service: Any | None = None,
) -> None:
    """Update renditions for an item.
    ...
    try:
        ...
        content, filename, content_type = await download_file_from_feeding_service_or_directly_async(
            href, feeding_service, **request_kwargs or {}
        )
        file_type, ext = content_type.split("/")
        metadata = process_file(content, file_type)
        file_guid = await app.media.put_async(content, filename=filename, content_type=content_type, metadata=metadata)
        rendition_spec = await get_renditions_spec_async()
        ...
        logger.exception(e)
        if file_guid:
            await app.media.delete_async(file_guid)
        raise


async def transfer_renditions(
    ...
    for rend in iter(renditions.values()):
        if rend.get("media"):
            local = await app.media.get_async(rend["media"])
            if local:
