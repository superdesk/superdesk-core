...
class DictCursorAsync(Generic[ResourceModelType]):
    ...
    def __init__(self, cursor: ResourceCursorAsync[ResourceModelType], **to_dict_kwargs):
        self._cursor = cursor
        self._to_dict_kwargs = to_dict_kwargs or {}
    ...
    async def __anext__(self) -> dict:
        ...
        if item is not None:
            return item.to_dict(**self._to_dict_kwargs)
        raise StopAsyncIteration

    async def next(self) -> dict[str, Any] | None:
        item = await self._cursor.next()
        return item.to_dict(**self._to_dict_kwargs) if item else None
    ...
    async def to_list(self) -> list[dict[str, Any]]:
        return [item.to_dict(**self._to_dict_kwargs) async for item in self._cursor]
