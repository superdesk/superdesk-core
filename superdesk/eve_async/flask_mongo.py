...
class FlaskMongoAsync:
    """
    Creates an async Mongo connection and database based on Flask configuration.
    """

    def __init__(self, app, config_prefix="MONGO"):
        if "pymongo_async" not in app.extensions:
            app.extensions["pymongo_async"] = {}

        if config_prefix in app.extensions["pymongo_async"]:
        ...
        app.extensions["pymongo_async"][config_prefix] = (cx, db)

    @property
    def cx(self) -> AsyncIOMotorClient:
        """
        Automatically created :class:`~pymongo.Connection` object corresponding
        to the provided configuration parameters.
        """
        if self.config_prefix not in current_app.extensions["pymongo_async"]:
            raise Exception("flask_pymongo_async extensions is not initialized")
        return current_app.extensions["pymongo_async"][self.config_prefix][0]
    ...
    def db(self) -> AsyncIOMotorDatabase:
        ...
        if self.config_prefix not in current_app.extensions["pymongo_async"]:
            raise Exception("flask_pymongo_async extensions is not initialized")
        return current_app.extensions["pymongo_async"][self.config_prefix][1]

    def get_db_from_app(self, app):
        if self.config_prefix not in app.extensions["pymongo_async"]:
            raise Exception("flask_pymongo_async extensions is not initialized")
        return app.extensions["pymongo_async"][self.config_prefix][1]
