...
class NewsML12Formatter(Formatter):
    ...
    def _format_news_management(self, article, news_item):
        ...
        SubElement(news_management, "FirstCreated").text = article["firstcreated"].strftime("%Y%m%dT%H%M%S+0000")
        SubElement(news_management, "ThisRevisionCreated").text = article["versioncreated"].strftime(
            "%Y%m%dT%H%M%S+0000"
        )
        ...
        if article.get(EMBARGO):
            ...
            SubElement(status_will_change, "DateAndTime").text = get_utc_schedule(article, EMBARGO).isoformat()
        else:
    ...
    def _format_news_component(self, article, news_item):
        ...
        for company in article.get("company_codes", []):
            ...
            SubElement(
                metadata, "Property", attrib={"FormalName": "Exchange", "Value": company.get("security_exchange", "")}
            )
    ...
    def _format_descriptive_metadata(self, article, main_news_component):
        ...
        if "anpa_category" in article:
            for category in article.get("anpa_category", []):
                SubElement(descriptive_metadata, "Property", {"FormalName": "Category", "Value": category["qcode"]})
    ...
    def _format_place(self, article, descriptive_metadata):
        ...
        for place in article.get("place"):
            if place.get("country") or place.get("state") or place.get("world_region"):
                ...
                if place.get("state"):
                    SubElement(location, "Property", {"FormalName": "CountryArea", "Value": place.get("state")})
                if place.get("country"):
                    SubElement(location, "Property", {"FormalName": "Country", "Value": place.get("country")})
                if place.get("world_region"):
                    SubElement(location, "Property", {"FormalName": "WorldRegion", "Value": place.get("world_region")})
    ...
    def _format_dateline(self, article, descriptive_metadata):
        ...
        if located:
            ...
            if located.get("city"):
                SubElement(location_elm, "Property", attrib={"FormalName": "City", "Value": located.get("city")})
            if located.get("state"):
                SubElement(
                    location_elm, "Property", attrib={"FormalName": "CountryArea", "Value": located.get("state")}
                )
            if located.get("country"):
                SubElement(location_elm, "Property", attrib={"FormalName": "Country", "Value": located.get("country")})
    ...
    def _format_media(self, article, main_news_component, media_type):
        ...
        for rendition, value in article.get("renditions").items():
            ...
            content_item = SubElement(news_component, "ContentItem", attrib={"Href": value.get("href", "")})
            ...
            if article.get(ITEM_TYPE) == CONTENT_TYPE.PICTURE:
                if value.get("width"):
                    SubElement(
                        characteristics, "Property", attrib={"FormalName": "Width", "Value": str(value.get("width"))}
                    )
                if value.get("height"):
                    SubElement(
                        characteristics, "Property", attrib={"FormalName": "Height", "Value": str(value.get("height"))}
                    )
                ...
                if get_filemeta(article, "width"):
                    SubElement(
                        characteristics,
                        "Property",
                        attrib={"FormalName": "Width", "Value": str(get_filemeta(article, "width"))},
                    )
                if get_filemeta(article, "height"):
                    SubElement(
                        characteristics,
                        "Property",
                        attrib={"FormalName": "Height", "Value": str(get_filemeta(article, "height"))},
                    )

                duration = self._get_total_duration(get_filemeta(article, "duration"))
                if duration:
                    SubElement(
                        characteristics, "Property", attrib={"FormalName": "TotalDuration", "Value": str(duration)}
                    )
    ...
    def can_format(self, format_type, article):
        ...
        """
        return format_type == "newsml12" and article[ITEM_TYPE] in {
            CONTENT_TYPE.TEXT,
