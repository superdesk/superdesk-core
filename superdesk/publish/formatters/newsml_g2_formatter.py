...
class NewsMLG2Formatter(Formatter):
        ...
        try:
            ...
            return [
                (
                    pub_seq_num,
                    self.XML_ROOT
                    + etree.tostring(news_message, pretty_print=True, encoding=self.ENCODING).decode(self.ENCODING),
                )
            ]
    ...
    def _format_item_set(self, article, item_set, item_type):
        ...
        SubElement(
            item, "catalogRef", attrib={"href": "http://www.iptc.org/std/catalog/catalog.IPTC-G2-Standards_25.xml"}
        )
    ...
    def _format_item_meta(self, article, item_meta, item):
        ...
        if article.get(EMBARGO):
            SubElement(item_meta, "embargoed").text = get_utc_schedule(article, EMBARGO).isoformat()
    ...
    def _format_content(self, article, news_item, nitf):
        ...
        if article.get(FORMAT) == FORMATS.PRESERVED:
            ...
            SubElement(content_set, "inlineData", attrib={"contenttype": "text/plain"}).text = inline_data
        elif article[ITEM_TYPE] in [CONTENT_TYPE.TEXT, CONTENT_TYPE.COMPOSITE]:
            inline = SubElement(content_set, "inlineXML", attrib={"contenttype": "application/nitf+xml"})
            inline.append(nitf)
    ...
    def _format_subject(self, article, content_meta):
        ...
        if "subject" in article and len(article["subject"]) > 0:
            for s in article["subject"]:
                if "qcode" in s:
                    subj = SubElement(
                        content_meta, "subject", attrib={"type": "cpnat:abstract", "qcode": "subj:" + s["qcode"]}
                    )
    ...
    def _format_geonames_place(self, place, content_meta):
        subject = self._create_subject_element(
            content_meta, place.get("name", ""), get_geonames_qcode(place), "cpnat:geoArea"
        )
        if place.get("state") and place.get("feature_class", "").upper() != "A":
            self._create_broader_element(
                subject, place.get("state"), get_geonames_state_qcode(place), "cptype:statprov"
            )
        if place.get("state") or place.get("feature_class", "").upper() != "A":
            self._create_broader_element(
                subject, place.get("country"), get_geonames_country_qcode(place), "cptype:country"
            )
    ...
    def _create_broader_element(self, parent, broader_name, qcode, concept_type="cpnat:abstract"):
        ...
        """
        if broader_name:
            broader_elm = SubElement(parent, "broader", attrib={"type": concept_type, "qcode": qcode})
    ...
    def _create_subject_element(self, parent, subject_name, qcode, concept_type="cpnat:abstract"):
        ...
        """
        subject_elm = SubElement(parent, "subject", attrib={"type": concept_type, "qcode": qcode})
    ...
    def _format_located(self, article, content_meta):
        ...
        if located and located.get("city"):
            located_elm = SubElement(content_meta, "located", attrib={"type": "cpnat:abstract", "qcode": "loctyp:City"})
    ...
    def _format_groupset(self, article, item):
        ...
        for group in article.get(GROUPS, []):
            attrib = {"id": group.get(GROUP_ID), "role": group.get(ROLE, "grpRole:%s" % group.get(GROUP_ID))}
            ...
            for ref in group.get(REFS, []):
                if ID_REF in ref:
                    ...
                    if RESIDREF in ref:
                        ...
                        archive_item = superdesk.get_resource_service(ARCHIVE).find_one(req=None, _id=ref.get(RESIDREF))
                        if archive_item:
    ...
    def _format_company_codes(self, article, content_meta, item):
        ...
        for company in article.get("company_codes", []):
            ...
            subject = SubElement(
                content_meta, "subject", attrib={"type": "cpnat:organisation", "literal": literal_name}
            )
    ...
    def can_format(self, format_type, article):
        ...
        """
        return format_type == "newsmlg2" and article[ITEM_TYPE] in {
            CONTENT_TYPE.TEXT,
