...
class StorageDump(superdesk.Command):
    ...
    option_list = [
        superdesk.Option(
            "-n",
            "--name",
            help=f'destination file or directory (default: "{DUMP_NAME}_<datetime>")'
        ),
        superdesk.Option(
            "--dest-dir",
            default=DUMP_DIR,
...
class StorageRestore(superdesk.Command):
    ...
    option_list = [
        superdesk.Option(
            "--keep-existing",
            action="store_true",
            help="don't clear collections before inserting items"
        ),
        superdesk.Option(
            "archive",
            help="file or directory containing the database dump"
        ),
    ]

    def run(self, keep_existing: bool, archive: Union[Path, str]) -> None:
...
class StorageStartRecording(superdesk.Command):
    ...
    option_list = [
        superdesk.Option(
            "-n",
            "--name",
            help='destination file (default: "{RECORD_NAME}_<datetime>)'
        ),
        superdesk.Option(
            "--dest-dir",
            default=RECORD_DIR,
        ...
        superdesk.Option(
            "-b",
            "--base-dump",
            help="base full dump from which the record must be started"
        ),
        superdesk.Option(
            "--force-db-reset",
            action="store_true",
    ...
    def run(
        ...
        dest_dir: str,
        description: Optional[str],
        base_dump: Optional[Union[Path, str]],
        force_db_reset: bool,
        full_document: bool,
        collections: Optional[List[str]]
    ) -> None:
        ...
        if version < (4, 0):
            raise NotImplementedError(
                "You need to use MongoDB version 4.0 or above to use the record feature"
            )
        metadata = {
            "started": now,
            "executable": sys.executable,
...
class StorageRestoreRecord(superdesk.Command):
    ...
    option_list = [
        ...
        superdesk.Option(
            "--skip-base-dump",
            action="store_true",
            help="do not restore base dump if any is specified"
        ),
        superdesk.Option(
            "record_file",
            help="file containing the record"
        ),
    ]

    def run(self, force_db_reset: bool, skip_base_dump: bool, record_file: Union[Path, str]) -> None:
...
class StorageMigrateDumps(superdesk.Command):
    ...
    def run(self) -> None:
        confirm = input(
            ...
            "result in data issues, be sure to have a copy of all important dumps and records first. Do you want to "
            "continue? (y/N) "
        )
        if confirm.lower() != "y":
        ...
        if not dump_path.is_dir():
            ...
            for idx, p in enumerate(dumps):
                print(f"{INFO}Restoring dump {idx+1}/{len(dumps)}")
                metadata = get_dump_metadata(p)
                StorageRestore().run(
                    keep_existing = False,
                    archive = p
