...
class CleanImages(superdesk.Command):
    ...
    def run(self):
        ...
        if app.config.get("LEGAL_ARCHIVE"):
            ...
            legal_archive_version_items = superdesk.get_resource_service("legal_archive_versions").get_from_mongo(
                None, query
            )
    ...
    def __add_existing_files(self, used_images, items):
        for item in items:
            ...
            if item.get("renditions", {}):
                used_images.update(
                    [str(rend.get("media")) for rend in item.get("renditions", {}).values() if rend.get("media")]
                )
