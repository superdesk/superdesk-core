...
def filter_expired_items(provider, items):
    ...
    try:
        ...
        logger.info('Received {0} articles from provider {1}, but only {2} are eligible to be saved in ingest'.format(
...
def update_provider(self, provider, rule_set=None, routing_scheme=None):
    ...
    try:
        ...
        for items in registered_feeding_services[provider['feeding_service']].update(provider):
