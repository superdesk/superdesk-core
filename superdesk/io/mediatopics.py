...
class MediaTopicsIndex:
    ...
    def _get_parent(self, topic):
        return 'medtop:' + topic.get('broader')[0].split('/')[-1] if len(topic.get('broader', [])) and \
               topic.get('broader')[0].startswith('http://cv.iptc.org/newscodes/mediatopic/') else None
    ...
    def get_items(self, language='en-GB'):
        ...
        for (_k, code) in self.media_topic_map.items():
            if not code.get('retired'):
                items.append({'qcode': code.get('qcode'), 'name': self._get_pref_label(code, language),
