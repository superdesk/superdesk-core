...
async def test_not_authenticated(prodapi_app, prodapi_client):
    ...
    async with prodapi_app.test_request_context("/"):
        for resource in RESOURCES:
            ...
            resp = await prodapi_client.get(url_for("{}|resource".format(resource)))
...
async def test_authenticated(superdesk_app, superdesk_client, auth_server_registered_clients):
    """
    ...
    prodapi_app = await get_test_prodapi_app()
...
@pytest.mark.parametrize("auth_server_registered_clients", [(("ARCHIVE_READ",),)], indirect=True)
async def test_bad_shared_key(superdesk_app, superdesk_client, auth_server_registered_clients):
    """
...
async def test_scopes_partial(issued_tokens, prodapi_app):
    """
...
async def test_token_expired(superdesk_app, superdesk_client, auth_server_registered_clients):
    """
