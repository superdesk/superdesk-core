...
async def test_readonly(prodapi_app_with_data, prodapi_app_with_data_client):
    """
    Ensure that `users` endpoint is readonly
    :param prodapi_app_with_data: prod api app with filled data
    ...
    async with prodapi_app_with_data.test_request_context("/"):
        for method, status in (("get", 200), ("post", 405), ("patch", 405), ("put", 405), ("delete", 405)):
            # we send a request
            resp = await getattr(prodapi_app_with_data_client, method)(url_for("users|resource"))
            # we get a response
...
async def test_excluded_fields(prodapi_app_with_data, prodapi_app_with_data_client):
    ...
    async with prodapi_app_with_data.test_request_context("/"):
        # list
        resp = await prodapi_app_with_data_client.get(url_for("users|resource"))
