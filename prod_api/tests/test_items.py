...
async def test_readonly(prodapi_app_with_data, prodapi_app_with_data_client):
    ...
    async with prodapi_app_with_data.test_request_context("/"):
        for method, status in (("get", 200), ("post", 405), ("patch", 405), ("put", 405), ("delete", 405)):
            # we send a request
            resp = await getattr(prodapi_app_with_data_client, method)(url_for("archive|resource"))
            # we get a response
...
async def test_excluded_fields(prodapi_app_with_data, prodapi_app_with_data_client):
    ...
    async with prodapi_app_with_data.test_request_context("/"):
        # list
        resp = await prodapi_app_with_data_client.get(url_for("archive|resource"))
...
async def test_renditions(prodapi_app_with_data, prodapi_app_with_data_client):
    """
...
async def test_filters(prodapi_app_with_data, prodapi_app_with_data_client):
    """
    Test filtering.
    :param prodapi_app_with_data: prod api app with filled data
    :param prodapi_app_with_data_client: client for prod api app with filled data
    ...
    async with prodapi_app_with_data.test_request_context("/"):
        ...
        resp = await prodapi_app_with_data_client.get(url_for("archive|resource", **params))
        resp_data = json.loads((await resp.get_data()).decode("utf-8"))
...
async def test_profile_label_in_output(prodapi_app_with_data, prodapi_app_with_data_client):
    """
    ...
    async with prodapi_app_with_data.test_request_context("/"):
        ...
        for item in items:
            if item.get("type") == "text":
                # Details call
                details_resp = await prodapi_app_with_data_client.get(
                    url_for("archive|item_lookup", _id=item["guid"]),
                )
                details_data = json.loads((await details_resp.get_data()).decode("utf-8"))
